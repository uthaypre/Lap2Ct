...
def main():
    ...
    try:
        ...
        ct_organs_mesh, ct_organs_bool = get_3dct(next(Path(config.ct).glob("*.nii.gz"), None), seg_config.ct_organ_classes, seg_config.ct_organ_colors)
        ...
        for frame in laparoscopy_frames:
            ...
            for organ_name, lap_organ in lap_organs_pcd.items():
                ...
                if organ_name in ct_organs_pcd:
                    ...
                    logger.info(f"Registering {organ_name} to CT organ")
                    ...
                    if len(reg_results.correspondence_set) < 5:
                        logger.warning(f"Not enough correspondences for {organ_name}, skipping registration")
                        continue
                    ...
                    logger.info(f"Registered {organ_name} successfully")
                    ### 4. Non-Rigid Registration ###
                    logger.info("Starting non-rigid registration")
                    ...
                    logger.warning(f"{organ_name} not found in CT organs, skipping registration")
            ...
            print(f"Camera pose for frame {frame.name}: {camera_pose_tracker[-1]}")
